...
func _get_agent_instructions() -> String:
	return (
		...
		+ "CRITICAL STEP EFFICIENCY:\n"
		+ "- If the user asks a QUESTION (not a task), answer it directly in ONE step. No tools needed.\n"
		+ "- Do NOT narrate plans, repeat system rules, or announce what you will do. ACT or ANSWER.\n"
		+ "- Do NOT output thinking/planning as visible text (Plan:, Confidence Score:, Constraint Checklist:, Goal:, Strategy:). Keep reasoning internal.\n"
		+ "- ONE step = ONE action batch or ONE direct answer. If no tools are needed, the loop ends.\n"
		+ "- When the user asks to find/download assets from the internet, ALWAYS call web_search immediately. NEVER tell the user to search manually.\n"
		...
		+ "Obsidian (optional): search_obsidian then read_obsidian_note for design docs in the user's vault (Config → Settings).\n"
		+ "External plugins (Terrain3D, etc.): inspect_plugin to discover API, then call_node_method or create_script. "
		+ "For TERRAIN GENERATION: create a GDScript that uses the plugin API for procedural heightmaps/painting — do NOT try to paint terrain through repeated call_node_method. "
